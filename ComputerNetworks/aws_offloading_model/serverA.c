...
void calculate_square(char* input, int maximum_bufsize)
{
    float x = atof(input);
    int ret = snprintf(input, maximum_bufsize, "%f", x*x);
    if(ret < 0)
    {
        fprintf(stderr, "Error in calculating square");
        exit(1);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    //Create UDP socket
    sockfd = bootup_UDP(&hints, &res, &p, "21396");
    ...
    while(true)
    {
        ...
        printf("The Server A received <%s>\n", input);

        // Processing the input value
        calculate_square(input, maximum_bufsize);
