...
void calculate_fifth_power(char* input, int maximum_bufsize)
{
    float x = atof(input);
    int ret = snprintf(input, maximum_bufsize, "%f", x*x*x*x*x);
    if(ret < 0)
    {
        fprintf(stderr, "Error in calculating 5th power");
        exit(1);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    //Create UDP socket
    sockfd = bootup_UDP(&hints, &res, &p, "23396");
    ...
    while(true)
    {
        ...
        printf("The Server C received <%s>\n", input);

        // Processing the input value
        calculate_fifth_power(input, maximum_bufsize);
