...
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
	...
	for epoch in xrange(max_epochs):
		...
		if epoch % lr_decay_every == 0 and epoch > 0:
			optimiztrain_neter.lr = optimizer.lr * lr_decay
		...
		#############################################################################
		train_acc = compute_acc(model, data_train, labels_train, num_samples=1000, batch_size=batch_size)
		...
		if val_acc > opt_val_acc:
			...
			opt_val_acc = val_acc
			opt_params = {}
			for k, v in model.net.params.iteritems():
				opt_params[k] = v.copy()
			#############################################################################
			#                             END OF YOUR CODE                              #
			#############################################################################
