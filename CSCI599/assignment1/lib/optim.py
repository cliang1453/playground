	...
	def step(self):
		...
		for layer in self.net.layers:
			for n, v in layer.params.iteritems():
				...
				mt_corrected = self.mt[n] / (1 - self.beta1**self.t)
				vt_corrected = self.vt[n] / (1 - self.beta2**self.t)
				layer.params[n] = layer.params[n] - (self.lr * mt_corrected) / (np.sqrt(vt_corrected) + self.eps)
