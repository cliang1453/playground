...
class LSTM(object):
	...
	def step_forward(self, x, prev_h, prev_c):
		...
		i = sigmoid(A[:, :self.h_dim])
		f = sigmoid(A[:, self.h_dim:2*self.h_dim])
		o = sigmoid(A[:, 2*self.h_dim:3*self.h_dim])
		g = np.tanh(A[:, 3*self.h_dim:])

		next_c = f*prev_c + i*g
		next_h = o*np.tanh(next_c)
		meta = [i, f, o, g, next_c, prev_c, prev_h, x]
	...
	def step_backward(self, dnext_h, dnext_c, meta):
		...
		#############################################################################
		i, f, o, g, next_c, prev_c, prev_h, x = meta
	...
	def forward(self, x, h0):
		...
		h = np.zeros((x.shape[0], x.shape[1], h0.shape[1]))
		c = np.zeros(h.shape)
		c0 = np.zeros((h0.shape[0], h0.shape[1]))
	...
	def backward(self, dh):
		...
		T = dh.shape[1]
		H = dh.shape[2]
		D = self.meta[0][7].shape[1]
		#i, f, o, g, next_c, prev_c, prev_h, x
...
class word_embedding(object):
	...
	def forward(self, x):
		...
		N: batch size
		T: the total number of words in the sentence of every batch 
		(how many word is blank for us to predict in a sentence)
		V: the total number of words in the dictionary 
		(how many choices of word we can predict from)
		D: embedding vector dimension 
		(for each word in the dictionary associate with a corresponding word vector, who has the dimension of D)

		x: integer array (N, T) gives indices of words. Each element idx
		  of x muxt be in the range 0 <= idx < V.
		  N batches of need to predict sentencs. each sentence contains T words to predict. 
		  The value at each entry of x at (n, t) is the entry number in the word dictionary of that word
		
		self.params[self.wx_name]: weight matrix of dimension (V, D). giving word vectors for all words.
		  V numbers of words in the dictionary. For each word in the dictionary, a D dimensional vector associates.


		out: array of embedding vectors (N, T, D) giving word vectors for all input words.
		  extract the corresponding word vector from the word dictionary W for each entry number in x at position (n,t). 
		  out(n, t, :) = the word vector of the word at x(n, t)
	...
	def backward(self, dout):
		...
		# no.add.at: 
		# According to the number of frequency of each word prediction in x (how many times each entry
		# appears in x), we repeatedly add that number of the word vector corresponding to the entry as the gradient
		# of W.
		# Given dW(V, D), x(N, T), dout(N, T, D):
		# for n, t in (N, T):
		#		dW(x(n, t), :) +=  dout(n, t, :)
...
class temporal_fc(object):
	...
	def forward(self, x):
		...
		Extract the corresponding word vector from the word dictionary W for each entry number in x 
		at position (n,t). x(n, t, :) = the word vector of the word corresponding to the value at x(n, t)
