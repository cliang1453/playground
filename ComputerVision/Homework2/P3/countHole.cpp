...

{
	...
	int** mask2 = new int*[height+2];
	for (int i = 0; i < height+2; i++)
	{
		mask2[i] = new int[width+2];
	}
	...
	//set mask 2 background as 1, foreground (including the holes) as 0)
	labelBlack(data, mask2, height+2, width+2);

	//set original data background as 1, foreground (excluding the holes) as 0
	for (int i = 0; i < height+2; i++)
	{
		for (int j = 0; j < width+2; j++)
		{
	...
	//set original data bakcground as 0
	for (int i = 0; i < height+2; i++)
	{
		for (int j = 0; j < width+2; j++)
		{
			if(mask2[i][j] == 1 && data[i][j] == 1)
				data[i][j] = 0;
		}	
	}
    ...
    //=====================count number of dots and fill the hole
    int numDots = 0;
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
	...
	cout<<"The total number of hole is "<<numDots<<endl;
	return 0;
}