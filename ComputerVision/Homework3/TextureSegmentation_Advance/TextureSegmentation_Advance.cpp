...
void kmean_advance(Mat features, int cluster_num, vector<vector<int>> &originalCluster, Mat &result_Mat, int turn)
{
	...
	{
		findCentroids_advance(indices, features, centroids.col(i), centroids.col(i+1));
	}
	...
	while (true)
	{
		...
		else
		{
			...
			for (int k = 0; k < cluster_num; k++)
			{
				...
				for (int j = 0; j < indicesCluster[k].size(); j++)
				{
					result_Mat.at<double>(int(indicesCluster[k][j] / width), indicesCluster[k][j] % width) = color[k + turn * cluster_num] * 255;
				}
			}
			cout << "converge, break" << endl;
			break;
...
void bfsHelper(Mat& Fillhole_Results, Mat& mask, queue< pair<int, int> >& colorQueue, const int colorValue, int& count)
{
	while (!colorQueue.empty())
	{
		pair<int, int> temp = colorQueue.front();
		colorQueue.pop();
		count++;
		int i = temp.first;
		int j = temp.second;
		...
		for (int a = 0; a<4; a++)
		{
			...
			{
				if (mask.at<double>(i + deltai[a],j + deltaj[a]) == 0 && Fillhole_Results.at<double>(i + deltai[a],j + deltaj[a]) == 0)
				{
					mask.at<double>(i + deltai[a],j + deltaj[a]) = 1;
					colorQueue.push(make_pair(i + deltai[a], j + deltaj[a]));
				}
			}
		}
	}
					...
					for (int p = 0; p<height; p++)
					{
						for (int q = 0; q<width; q++)
						{
							if (mask.at<double>(i, j) == 1)
								Fillhole_Results.at<double>(i, j) = filledColor;
						}
					}
				}
...
int main(int argc, char *argv[])
{
	...
	writeSegmentedImage(Fillhole_Results);
	cout << "CHECK POINT 7 FINISHED" << endl;
